...
Platform and rendering targets:
- Native desktop support: **Windows + Ubuntu (first Linux target)**.
- Rendering priority: **Vulkan-first**.
...
6. **UX-first for non-coders**
   - Default workflows and labels must remain understandable without technical knowledge.
   - Advanced controls should be accessible but not intrusive.
...
- If AI cannot reliably generate requested content:
  ...
  - allow manual upload.
- If AI fails the same requested operation **5 times in a row**, auto-offer guided manual mode while preserving a “try AI again” option.
...
- Allowed by default:
  - CC0 / Public Domain
  - CC-BY 4.0 (only when attribution can be exported automatically)
  - User-uploaded assets with explicit ownership/rights confirmation
- Blocked in V1:
  - CC-BY-SA
  - CC-BY-NC
  - Any asset with missing/unclear license metadata
- Export requirements:
  - Keep per-asset license metadata
  - Generate attribution bundle automatically for eligible assets
  - Block publish when required rights/attribution data is missing
...
### Publish Audit Trail
- Generate an audit trail automatically during publish preparation.
- On Steam publish flow, allow upload/export of audit data with explicit user confirmation.
- Provide redaction controls for sensitive fields (for example: local paths, secrets).
- Keep a local signed/hashable copy for tamper-evident recordkeeping.
...
- Native desktop targets: Windows + Ubuntu.
...
## 18) Project/Repo Defaults
- Git integration is optional in V1.
- Default behavior for new projects: Git is **OFF**.
- Provide a clear in-app “Enable Git” action for users who want repository tracking.

---

## 19) Quality and Release Thresholds (V1 Defaults)
- Crash-free session rate target: **>= 97%**.
- Performance target: 60 FPS target in core gameplay scenes.
- Critical performance fail condition: sustained below 30 FPS on target-spec validation scenes.
- Frame-time stability target: 95th percentile frame time below 33ms in core gameplay flows.
- Initial scene load target on target hardware: below 20 seconds.
- Save/load integrity in regression checks: 100% pass required.
- Quest/progression blocker dead-ends: 0 known critical blockers at release candidate.

---

## 20) Non-Negotiables Recap
