...
Paths under `~/.cache/soul-loom/` and environment variables **`SOUL_LOOM_*`** are defined in `model_manager.py` and `models/manager.py`.
...
2. Detects NVIDIA VRAM (`nvidia-smi`) or uses `SOUL_LOOM_GPU_VRAM_GB` override.
