#!/usr/bin/env python3
"""Core orchestration logic: pipelines, assets, playtests, and CLI dispatch.

Entry point for subprocess callers is :file:`../orchestrator.py`, which re-exports
this module's public names for compatibility with importlib-based tests.
"""
...
from orchestration.io_utils import _slugify, _utc_now_iso, _write_json, _write_text
from orchestration.regeneration import apply_partial_regeneration
from orchestration.types import (
    ActionablePlaytestReport,
    AssetCatalogEntry,
    AssetImportError,
    ...
    BotPlaytestResult,
    BotPlaytestScenario,
    ConsequenceTransitionResult,
    GeneratedGraphicAssetResult,
    OperationFailureFallbackState,
    OperationFailureTracker,
    OrchestrationResult,
    PipelineExecutionResult,
    PipelineStageResult,
    PipelineStageStatus,
    PlaytestReportSection,
    ProjectStyleSelectionState,
    RegenerationConflict,
    RegenerationResult,
    STYLE_BUILTIN_PRESETS,
    StylePresetDefinition,
)
from orchestration.uncertainty import generate_think_for_me_directions, generate_uncertainty_options
        ...
        suggestions.append(
            {
                ...
                "summary": (
                    f"LoomGuard recommendation for target '{target_profile}' from perf history + change log. "
...
def _execute_generation_pipeline(
    ...
    *,
    ...
    benchmark_result = run_benchmark_as_dict(orchestrator_file=ORCHESTRATOR_SCRIPT_PATH, auto_prepare_models=True)
    ...
    if command in {"onboarding-run", "/onboarding_run"}:
        try:
            result = run_onboarding(
                orchestrator_file=ORCHESTRATOR_SCRIPT_PATH,
                progress_callback=emit_progress if progress_json else None,
                cancel_check=is_cancelled if cancel_file_path else None,
            )
            print(json.dumps(result, indent=2))
...
def main() -> int:
    ...
    if args.prepare_models:
        try:
            result = prepare_models_as_dict(orchestrator_file=ORCHESTRATOR_SCRIPT_PATH)
            print(json.dumps(result, indent=2))
            return 0
        except Exception as exc:  # noqa: BLE001 - keep subprocess output structured
            print(
