...
- Use locked V1 stack:
  ...
  - Editor/tooling UI: C#
  - AI orchestration/automation: Python
- Target OS support for V1: Windows and Ubuntu.
...
## Milestone 0 — UX Foundation (run before implementation-heavy milestones)

### Prompt M0-P1: UX wireframe baseline
```text
Task: Create and validate V1 UX wireframe baseline
...
Deliverables:
1) Wireframe artifacts (markdown/image references).
2) Navigation flow map.
3) UX assumptions and open questions list.

Acceptance Criteria:
- Core flow from idea -> prototype -> editor -> testing -> publish is represented.
- Advanced controls are secondary to simple controls in the default layout.
```

### Prompt M0-P2: UX content and labeling pass
```text
Task: Define plain-language labels and confirmation copy for core actions

Goal:
Reduce cognitive load for non-coders by standardizing naming and confirmation text.

Deliverables:
1) UX copy table for buttons, dialogs, tooltips, and warnings.
2) Confirmation templates for destructive actions and publish warnings.
3) Mapping between technical terms and user-facing language.

Acceptance Criteria:
- Core actions use clear verbs and non-technical wording.
- Destructive/critical actions include explicit impact summaries.
```
...
- Includes startup verification notes for both Windows and Ubuntu.
...
4) License allow-list enforcement and metadata capture at import.

Acceptance Criteria:
- Imported assets become searchable by tags/category.
- Failed imports return actionable errors.
- Unsupported/unclear licenses are blocked with explicit error reasons.
```
...
4) Numeric thresholds integrated (crash/perf/load-time/safe-save checks).
5) Audit trail generation, local signing/hash, and user-confirmed upload flow.

Acceptance Criteria:
- Critical issues prevent publish action.
- Warnings allow override with explicit acknowledgement.
- Publish flow surfaces audit report and consent before external submission.
```
